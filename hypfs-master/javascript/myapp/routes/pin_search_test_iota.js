...
const filepath_DHT = "C:/Users/Amministratore/Desktop/IOTA_DHT/hypfs-master/javascript/myapp/test_files/pin_DHT/pin_" + config.dht.HIPERCUBE_SIZE + ".csv"
...
router.post('/pin_search_iota', async function (req, res) {
    ...
    pinStartTime = new Date().getTime()

    make_req(encoded_point, async function (data) {

        pinEndTime = new Date().getTime()
        //utils.write_csv(pinStartTime, pinEndTime, filepath_DHT)
...
const make_req = async function (keyword,  callback) {
    ...
    const options = {
        ...
        method: 'GET',
        qs: { 'keyword': keyword,  'sender': 'user' },
        json: true
    };
